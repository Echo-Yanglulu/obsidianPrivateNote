...
1. 是否副作用
	...
	2. [[非纯函数]]：
2. 功能
	1. [[构造函数]]：通过new关键字调用，用于创建一个新对象的函数
	2. 生成器函数：function*
...
1. **对象**：可进行保存（变量，对象属性，数组元素）、传递（作为参数）、添加属性
	1. 场景：函数需要一个静态变量，且*每次调用时都可访问*。（只有自己会用到的一个值。比如期望每次返回一个不同的值，可用属性保存返回过的值）
	2. 定义接收函数作为参数的函数时：在特定条件下调用、传参。
...
4. 对作用域的影响
	...
	2. 创建新[[作用域]] 
		1. 函数体内声明的变量在函数外部不可见。
			1. 函数作为[[命名空间]]。保存一段代码：可能被多个地方用到、不希望与那些调用环境产生变量冲突
		2. [[闭包]]🐇：函数内部状态，除了代码，还有它定义时所在作用域的引用
			1. 定义时用于传入的函数时：接收官方 API/三方库传入的值，进行处理、返回
> [!NOTE] Title
...
# 属性
### name
只读，定义函数时使用的名字。如果是没有命名的函数，则是第一次创建时它赋予的[[变量]]或属性名。
### length
### prototype
### \[\[Scpoe]]
### \[\[Location]]
### arguments
存在条件
	1. 使用function关键字创建的函数中存在
		1. 箭头函数中没有，但可通过参数收集模拟
内涵：函数被调用时的传参
本质：[[类数组对象]]，
特性
...
| --- | --- | --- | --- | --- |
| [[定义法]] | 4个都必须 | 函数名可选 | 函数名、function可选 |  |
| 场景 |  |  |  |  |
...
6. 递归
7. 尾递归：函数尾调用自身
8. 尾调用：函数的最后一步是调用另一个函数`function f(x) { return g(x); }` 
