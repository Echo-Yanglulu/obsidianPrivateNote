...
|301|	Moved Permanently（永久移除）|	请求的 URL 已移走。响应中应包含一个 Location URL，说明资源现在所处的位置。浏览器自动处理
|302|	Move Temporarily（已找到） |	移除是临时的。客户端应该用 Location 首部给出的 URL 对资源进行临时定位。浏览器自动处理
