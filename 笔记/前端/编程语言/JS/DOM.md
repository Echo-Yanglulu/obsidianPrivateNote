...
文档中的所有节点都与其他节点有关系
每个节点都有一个 
	1.  `childNodes` 属性，其中包含一个 [[NodeList]] 的实例
		1. childNodes列表中的每个节点都是同一列表中其他节点的*同胞*节点
		2. previousSibling和nextSibling可以在这个列表的节点间*导航*
		3. 列表中第一个节点的previousSibling属性、最后一个节点的nextSibling属性是null
		4. 只有一个节点，则它的previousSibling和nextSibling属性都是null
