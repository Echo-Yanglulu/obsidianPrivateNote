...
终极window属性，始终指向window：self对象
	实际上self和window对象是同一个对象，暴露self只是为了和top, parent保持一致
可通过三个属性，把多个窗口的window对象串联起来。
	1. 为啥没有子窗口？只有相对上层和绝对最上层？*不需要向下或向旁边*？
...
物理分辨率：屏幕实际的分辨率
CSS像素：浏览器报告的虚拟分辨率
window.devicePixelRatio：物理分辨率/CSS像素
	1. 与每英寸像素数（DPI，dots per inch）是对应的

背景：低分辨率屏幕与4K分辨率屏幕下，12CSS像素的字体应该有肉眼看起来有同样大小。为此提出了**CSS像素**。
