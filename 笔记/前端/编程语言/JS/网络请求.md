...
2. xhr.readyState：当前连接的状态
	- 0：unset open方法未调用【未初始化请求】
	- 1：opened open方法已调用【已初始化请求】
	- 2：headers_received *已收到头部*【已收到响应头】
	- 3：loading *正在下载*，resonseText已有部分内容【已收到内部响应体】
	- 4：done *下载完成*
...
ontimeout
### overrideMimeType
**重写XHR响应的MIME类型**。响应的XHR类型决定了*XHR对象如何处理该响应*，所以覆盖响应的MIME类型是很有用的。
# 跨源资源共享
主题：[[前端安全]] 
背景：XHR存在限制，主要是跨域安全策略，[[同源策略]]。但浏览器也需要进行合法跨源的能力
核心：服务端设置[[HTTP Header]] 
CORS：规定了浏览器与服务器如何实现跨源通信【OHMC】
...
定义
	...
	2. 内容类型是text/plain, multipart/form-data, application/x-www-form-urlencoded
...
### 预检请求
非 GET/POST 的[[HTTP Method]]、使用*自定义头部*，不同*请求体内容类型*。
		1. 某个涉及以上某个高级选项时，会先发一次送预检请求[^1] 
		2. 预检请求响应
			1. allow origin
			2. allow headers
			3. allow method
			4. allow creadentials
			5. max age：预检请求的响应会被缓存
特点
	1. 浏览器自行发起的，无需开发干预
为何要预检请求？
	1. 跨域请求之前的预检查，检测服务端是否有满足当前请求的能力
### 凭据请求
...
2. 特点
	1. 是浏览器提供的规范底层 API
	2. 有XHR对象的所有功能，但更容易使用，接口更现代化（接口更现代化是什么意思）。是JS**请求资源**的优秀工具
	3. 返回 [[Promise]] 
		1. 请求失败不会调用reject。网络故障、请求被阻止时才会
	4. 默认情况，fetch不会从服务端发送或接收cookies
		1. 要发送cookie进行论证必须设置credentials选项。
			1. 默认为same-origin
3. 兼容性：[fetch() - Web API 接口参考 | MDN](https://developer.mozilla.org/zh-CN/docs/Web/API/fetch#浏览器兼容性) 
	1. 只有Firefox和safari支持Authorization字段
4. 场景
