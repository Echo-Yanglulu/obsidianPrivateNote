...
`. ` 
	1. :回车（\\r）、换行(\\n) 、行分隔符（\\u2028）和段分隔符（\\u2029）以外的所有字符
	2. 码点大于0xFFFF 的 Unicode 字符，点字符不能正确匹配，会认为这是两个字符
### 数量【贪婪重复】
定义：某个字符/字符类可能重复多少次
	1. 量词
		1. {n,m}
...
1. 任选：|
	1. 多个模式之一。左侧匹配后忽略右侧。
2. 分组并引用：()
	1. 可提取出与子模式对应的部分匹配项，用数字表示
	2. 引用：引用之前的子模式所选出的*匹配项* 
		1. 形式：\\数值。\1代表引第一个子模式的匹配项
		2. 存在模式嵌套时，以左括号为准
		3. 作用：强制在模式中的不同部分包含相同文本。
3. 分组但不引用：(?:...)
	1. 如果()内以?:开头，则不会产生引用，而是将多个连续的字符解释为一个整体。
案例
	1. /\['"]fjiwoe\\1/：强制结尾必须匹配开头的引号。

### 位置
匹配字符间的位置，而不是字符
	1. \\b：匹配[[ASCII]]词边界。即\\w(**ASCII单词字符**)与\\W(**非单词字符**)的边界，或*ASCII单词字符*与字符串开头或末尾的边界。例外：在字符类中的 \\b匹配退格字符
	2. /^...$/：独占一行的字符串
	3. 
## 字面量字符【一个】
1. 字母数字字符
...
1. 形式
	...
	2. 转义（可也出现于方括号中）
		...
		2. \\s与\\S：匹配任意[[Unicode]]空白字符。空格字符、制表字符
