...
定义
	1. ECMA-262：一组属性的**无序集合** 
	2. 一种**复合值**（汇聚多个值），允许按名字读写这些汇聚的值
	3. 通常说：把字符串映射为值
范围
...
指定[[原型对象]]地创建新对象是一种很强大的特性
	...
	3. 如果不传。报TypeError：必须是对象或null
## 多个创建
> [!note] 批量创建
> 问题：创建具有同样接口的多个对象时，需要编写重复代码：[[JS创建对象]] 
...
尝试
	...
	2. 仅覆盖目标对象存在的属性
## 序列化
定义：把对象转换为字符串的过程，并可恢复为对象。
方式
	1. [[JSON]] 
# 属性【读取，设置，删除，测试，枚举】
## 形式
属性之间逗号，key 与 value冒号
	1. 属性名
...
### 过程
[[原型对象]] 
==原型链==：自有属性 ||[[ 原型对象]]上的属性 || 原型对象的原型对象的属性 ||  ... || null
	如何上溯？通过原型对象的__protot__属性
instanceof 是基于原型链实现
### 结果
1. 属性不存在：undefined（在自有属性、继承属性中没有）
2. 对象不存在：报错
3. 没有属性的 null 或 undefined：报 TypeError
## 设置
### 语法
将读取的语法放到赋值表达式左侧
obj.a
...
1. 赋值成功
2. 赋值失败
	1. 对象的 extensible 特性为 false，则不可新增
	2. 自有或继承属性writtable为false，则不可修改
...
## 测试（某个）
1. 是否存在某个**自有**属性： `prototype.hasOwnProperty` 
	1. 只查看自有
2. 是否存在某个**自有或继承**属性：属性名（[[String]], [[Symbol]]） `in` 对象名
	1. 会查看原型链
3. 是否存在某个**自有且可枚举**属性： `prototype.propertyIsEnumerable` 
	1. for/in时可通过该方法排除继承的可枚举属性。

in可做到而属性访问判断做不到的：in可判断是否存在。访问断言只能判断是否为undefined
## 枚举（某些）
### 方式
1. 所有：Reflect.ownKeys()
2. 自有符号：Object.getOwnPropertySymbols()
3. 自有字符串：Object.getOwnPropertyNames()
	1. 不要再使用 keys 去枚举了，可能遗漏不可被枚举的属性。
4. 可枚举字符串：for/in
	1. 枚举顺序不确定
5. 自有可枚举字符串：Object.keys()
	1. 枚举顺序不确定
	2. Object.values()
	3. Object.entries()
### 顺序
