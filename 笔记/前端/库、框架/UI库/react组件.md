...
4. 非嵌套关系
	1. 手动：父向子通过 props 下传 setState
	1. 自定义事件。使用三方库： `events` 
# 逻辑复用
...
1. 区别
	...
	2. 代码组织
		1. 以生命周期为核心组织代码，一个功能的相关代码需要分布在挂载、更新、卸载这三个生命周期函数中定义。
		2. 以功能/[[副作用]]为核心组织代码，一个功能的相关代码定义在一个副作用 hook 里即可。
	3. 状态同步
		1. 类组件：生命周期中执行某个函数时，拿到的 state 是最新值
		2. [[函数组件]]：捕获。函数组件中使用闭包，执行某个函数时，拿到的 state 是某次渲染的捕获值。
	4. 逻辑复用
...
[^1]: 受开发者控制的组件
[^2]: 类组件中使用它获取元素，然后 focus，但后续维护不方便，比如想给需要 find 的元素添加一层 div，就无法找到了。所以使用函数组件是优先的选择。