...
## 匹配规则
默认/多个：模糊匹配。只要路由部分的左侧出现了path，就会渲染对应组件【会导致同时渲染多个组件】
单个：Switch包裹Route。只渲染第一个匹配path的组件。
精确：Route添加exact属性。精确匹配时才渲染该组件
## 路由切换
方式
	1. 使用a标签。除了导致渲染其他组件，也会刷新页面【不是[[SPA]]】。
	2. 使用Link组件。不刷新整个页面地加载所需组件。