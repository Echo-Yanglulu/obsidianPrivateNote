...
# useEffect
## 原理
1. 会**捕获**props与state【在副作用设置函数里，拿到的是初始的props与state。如果想拿到最新值】
	1. 添加到监听队列，会自动重新捕获队列中所有值。【仅当副作用需要依赖该state时】
	2. 使用useRef重新捕获，但通常有其他更简单的实现方式
	3. 在副作用的setState中传入函数。
2. 副作用执行需要读取最新的state，但副作用的执行不依赖于它的改变。【则副作用执行时拿到的是始终旧的state】如定时器
