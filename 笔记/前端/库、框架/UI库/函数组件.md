...
## 执行机制
1. 每次渲染
	1. 再次调用useState、useEffect，生成一组只属于此次渲染的、始终不变的（state+props+effect+eventHandler）
		1. useState会使用setState传入的值，作为最新值返回
		2. useEffect会使用全新的props，state传入一个新副作用函数
	2. 执行return，读取此次执行时对所有数据所生成的快照，并依据该快照生成UI的快照。
2. 渲染完毕
	1. 需要副作用
		1. 执行副作用清除【因为大多数effect的执行并不会阻塞DOM更新】
		2. 执行副作用添加
	2. 跳过副作用【避免不必要地添加副作用】
# useState
...
执行机制
	1. 每次渲染时执行。
		1. 将初始值（挂载）或通过setState传入的值（更新）声明为当前组件内的常量
## 原理
