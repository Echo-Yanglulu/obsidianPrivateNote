...
内部机制
	...
	3. 存在依赖
		...
		3. 依赖更新
			...
			2. 是此次渲染时捕获的添加逻辑
执行机制：捕获变量，副作用的添加逻辑在*此次渲染后*执行，副作用的清除逻辑在
...
结论：
	...
	2. 如果要操作 DOM，或者不想出现 **内容闪烁** 的问题，我们就是用 useLayoutEffect
# useRef
...
## 应用
1. 避免重渲染：当子组件的属性为函数时，使用useCallback（减少渲染次数）
# useLayout
...
# useImperativeHandle
目的：父组件获得子函数组件的**状态和方法** 

对比：forwardRef结合ref是拿到子函数组件中的**元素** 
# 自定义hook
```js
