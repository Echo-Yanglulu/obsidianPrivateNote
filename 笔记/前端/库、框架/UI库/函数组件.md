# 概述
期望一个纯函数，（react只需从上到下执行它），接收相同参数（props、state 和 context），返回相同的JSX
目的：为了在函数组件中使用[[状态]]与[[副作用]]（函数组件曾被区别称为“无状态组件”）。
特点
	1. [[纯函数]]，输入 props，输出[[JSX]] 。
		1. 执行完即销毁，无法存储 state。通过 statehook,把 state 功能添加到纯函数中
	2. 没有实例、生命周期、state
	3. 不能扩展其他方法
[[class组件]] 的问题
	1. 大型组件很难拆分，重构，测试（即 class 不易拆分）
	2. 相同的业务逻辑分散的各个生命周期中，混乱
	3. 逻辑复用复杂。mixins, hoc, render-props
意义
	1. 发明 hooks 的动机
		1. 函数组件的重构。在无状态组件需要状态时，不需要完全重构为类，可直接使用状态管理、副作用
		2. 逻辑复用
	2. 它如何改变了react
	...
	3. 它解决了哪些问题？
	4. 如何模拟 [[class组件]] 的生命周期？
前提
	1. 如果不理解[[Function]]、[[纯函数]]、[[副作用]]，就无法理解react处理函数组件的方式。
...
特点
	1. 每次渲染，都是独立的闭包。
		1. 拥有属于此次渲染的（state+props+eventHandler+组件UI+逻辑）
		2. 捕获的值不会受后续操作的影响
# useState：状态
...
形式：const [state, setState] = useState(initialValue)
	...
	3. initialValue：状态的初始值
		1. [[Function]]：惰性初始化：初始化时调用，设置返回值为初始值
		2. [[值]] 
执行机制
...
const \[state, dispatch] = useReducer(reducer, initialArg, initialFunc?)
功能：给组件添加一个reducer
使用
	1. reducer返回一个新对象，用于更新state
		1. state只读，不要修改它的属性
		2. 返回一个新对象，包含解构后的state和新属性
	2. 初始化
		1. 如果初始值需要经过计算，不要在第二个参数直接传入函数，因为后续更新也会调用，即使不生效
		2. 第三个：用于初始化值的函数
		3. 第二个：初始化函数需要使用的参数
	3. 调用dispatch时传入的对象，会成为reducer调用的第二个参数
...
另一种useState。与它有什么区别？？？
	...
	2. 不同
		1. **使用reducer管理状态**。将state改变时相关逻辑封装在修改方法内
...
# useId
功能：生成独特的ID，传递给 `HTML辅助功能属性` 
HTML辅助功能属性：指定两个标签彼此相关。如，一个p元素描述另一个input元素的输入格式要求。
`const id = useId()` 
	1. 不需要参数
	2. 返回一个唯一值，
硬编码不是一种好的实践：[[硬编码关联元素.png]] 因为组件可能被调用多次，而ID必须是唯一的！
	1. 如果是表单组件所需的ID，会导致重复。
## 场景
1. label标签与input标签。htmlFor属性与id属性绑定。点击label会自动focus输入框
2. 在一个项目中渲染多个react应用
# useParams
获取动态路由参数
# useEffect
需要在页面渲染后执行的[[副作用]] 
	1. 每次渲染都会执行。即使用\[]，也会在刷新页面时执行该副作用
> [!功能] 

目的
	1. 组件挂载/依赖更新/没有依赖时，对组件内状态进行**捕获并排队**。组件渲染后，先**执行**上次渲染捕获的清除逻辑，再执行此次渲染捕获的添加逻辑。
	2. 组件卸载时，执行上次渲染捕获的清除逻辑
内部机制
	1. DOM 更新后执行
	2. 没有依赖：每次渲染时执行[[副作用]] 
	3. 空数组：**不使用数据流中的值**，只在初次渲染时执行
		1. 挂载时：保存添加与清除逻辑。渲染完成后执行添加逻辑，卸载时执行清除逻辑
		2. 更新时：不执行
		3. 卸载时：执行清除逻辑
	4. 存在依赖。初次渲染时，后续必要的 state 更新时执行副作用。
		1. 挂载时：保存添加与清除逻辑。渲染完成后执行添加逻辑，卸载时执行清除逻辑
		2. 依赖未更新：不执行
		3. 依赖更新
...
### 项目初始化的相关逻辑
不要把仅在项目初始化时执行一次的逻辑这样定义，因为页面刷新也会导致执行多次
```js
function App() {
  // 🔴 禁止：把只需要执行一次的逻辑直接放在 Effect 中
  useEffect(() => {
...
#### 重置部分
不要依赖 props 更新某个 state。通过新增一个 state 保存 props 旧值，对比后执行 state 更新逻辑。
问题写法：
```js
function List({ items }) {
...
某个state的改变会引起其他操作（提示），有多个回调中需要修改该state
	1. 错误方案：将该 state 作为依赖，该操作作为副作用。因为每次页面刷新都会触发该操作！
	2. 应该提取公共逻辑，将修改 state 与副作用放在事件回调函数中
```js
...
## 使用
### 在内部定义、调用副作用
如果在外部定义副作用，则难以判断该副作用依赖了哪些state/props
### 副作用函数
不要使用async函数，因为react要求副作用函数返回一个函数/undefined，而不是promise对象【async函数会返回promise对象】
# useInsertionEffect
DOM变动前触发
# useLayoutEffect
渲染前同步执行的副作用
组件从 state 变化到渲染，大概可以分为如下几步
	1. **改变 state**，触发更新 state 变量的方法
	2. React 根据组件返回的 vDOM 进行 **diff 对比**，得到**新 Virtual DOM**
	3. 将新的 VDom 交给渲染线程处理，**绘制到浏览器上** 
useEffect 
	1. 在第 3 步之后执行
	2. 异步执行，所谓异步就是被 requestIdleCallback 封装，只在浏览器空闲时候才会执行，保证了不会阻塞浏览器的渲染过程。
useLayoutEffect
	1. 在第二步之后（diff 出新的 vDOM 之后），第三步之前执行，也就是渲染之前同步执行的，所以会等它执行完再渲染页面到浏览器上。
	2. 在浏览器layout之后，painting之前
结论：
	1. 与useEffect的区别
		1. 在渲染前执行，而不是渲染后
		2. 同步的，会阻塞DOM渲染。
...
# useContext：传递
功能：在函数组件中读取一个组件树中的[[context]]。
	1. 在某个组件中修改全局用到的数据
# useMemo：缓存
传参：同[[函数组件#useEffect]]。
功能：当依赖改变时，重新执行一次**大量计算**并返回一个新值
> [!note] 应用
> 
...
# useRef：保存不变值、传递
作用
	1. 保存页面渲染无关的数据，在组件的执行周期中始终不变。一般是DOM。
	2. 父组件获取子函数组件提供的**元素** 
特性
...
```
对比：forwardRef结合ref拿到子函数组件中的**元素**，再使用useImperativeValue可拿到**部分值**。
# useDebugValue：调试
功能：为自定义hook添加标签
useDebugValue(value, format?)
	1. value：想在 React *开发工具中显示*的值。可以是任何类型。
	2. 可选 format：格式化函数。当组件被检查时，React 开发工具将用 value 作为参数来调用格式化函数，然后显示返回的格式化值（可以是任何类型）。不传则会显示 value。
	3. 没有返回值。
适用：作为*共享库*一部分、具有*复杂的内部数据结构并且难以检查*的自定义 Hook 
# useSyncExternalStore
功能：订阅一个外界store
形式： `const snapshot = useSyncExternalStore(subscribe, getSnapshot, getServerSnapshot?)`
# useTransition
功能：不会阻塞UI地更新状态
	1. 将状态改变，作为一种过渡
形式：const \[isPending, startTransition] = useTransition()
	1. 
含义：过渡
场景
	1. 
...
## 可以实现的功能
1. *状态*及其更新：useState/useReducer
2. [[副作用]]：同步或异步：useEffect/useLayoutEffect
3. 长期保存不变的*值*：useRef
4. 保留引用类型的引用*值*，条件更新
	1. useMemo
	2. useCallback
5. 深层双向*传递数据*：React.createContext、useContext
	1. 深层修改高层数据，再辐射到其它深层组件
6. 向父组件*传递子组件*[^5] ：React.forwardRef、useRef
	1. 整个元素：ref
	2. 部分数据：useImperativeHandle
7. 迟延*更新*【组件的展示依赖于大量数据】
...
[^5]: 前提：使用forwardRef为函数组件添加ref属性。可传递子组件的元素、数据
