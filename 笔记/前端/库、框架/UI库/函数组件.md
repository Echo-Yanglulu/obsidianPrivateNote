...
## 执行
1. 挂载【组件挂载到DOM结构中】
	1. 顶级非hook代码、useState
		1. *初始值*作为最新常量返回，生成*只属于此次渲染*的（state+props+eventHandler）
	2. 渲染代码
...
2. 更新【组件更新】
	1. 顶级非hook代码、useState
		1. *setState传入值*作为最新常量返回，生成*只属于此次渲染*的（state+props+eventHandler）
	2. 渲染代码
	3. 清除副作用【因为大多数effect的执行并不会阻塞DOM更新，所以先清除】
	4. 不需跳过，则添加副作用。需跳过，则不添加。
3. 卸载
	1. 清除副作用
...
# useMemo
memo和useMemo类似，基于Object.is的浅比较，仅仅对非引用类型有效
原因：当一个组件更新时，即使子组件接收的props没有更新也会重新渲染。为避免没有变化的重渲染。
## 如果传递的属性是引用类型数据，子组件仍会更新，怎么办

将传递的引用类型数据也使用useMemo
```js
...
export default function CountDown() {
  ...
  // 为什么在这里读取的就是更新后的值
  // 因为副作用是定时器，定时器执行过程中state是不断变化的，放到ref中
    if (timerNum > 0) {
...
# 总结
1. useMemo：将传入的**值**保存，避免重新生成
	1. 被memo的组件可避免*属性未改变*时重新渲染
		1. 如果属性传递的引用类型，子组件仍会更新。
			1. 使用useMemo保存该引用类型：`const info = useMemo(() => ({ name, age }), [name, age]);`[^2] 
	2. 不能用于函数。否则会报错：过多re-render
2. useCallback：将传入的**函数**保存，避免重新生成
	1. 避免每次组件更新都生成新的函数
		1. 向已memo的子组件传递回调
3. useCallback是useMemo的子集，因为函数也是值，
