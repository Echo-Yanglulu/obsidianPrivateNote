...
2. 更新【组件更新】
	1. 执行非渲染代码
		...
		2. useEffect【如果被**跳过**，则不执行】
			1. 如果没有被跳过，则重新捕获所有依赖变量，重新添加需要被执行和清除的副作用
	2. 执行渲染代码
	3. 渲染完毕
	4. 
	5. 执行上次执行useEffect时添加的清除副作用【因为大多数effect的执行并不会阻塞DOM更新，所以先清除】
	6. 执行上次执行useEffect时添加的副作用【如果被跳过，则不执行】
3. 卸载
	1. 清除副作用
