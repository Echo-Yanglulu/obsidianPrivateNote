...
1. 挂载【组件挂载到DOM结构中】
	1. 顶级代码
		1. 调用useState、useEffect，生成一组只属于此次渲染的、始终不变的（state+props+effect+eventHandler）
			1. useState会使用setState传入的值，作为最新值返回
			2. useEffect会使用全新的props，state传入一个新副作用函数
	2. 渲染代码
		1. 执行return，读取此次执行时对所有数据所生成的快照，并依据该快照生成UI的快照。
	3. 添加[[副作用]] 
	4. 清除副作用的函数进行捕获[^1]
2. 更新【组件更新】
	1. 顶级代码
...
形式：const [state, setState] = useState(initialValue)
	...
	2. setState：保存*修改状态的函数*的变量
		1. [[Function]]：去获取当前组件中维护的最新的状态值（并不是此次渲染的值）
			...
			2. **依赖react中维护的值，而不是此次渲染中的值**。即使当前渲染的值并没有更新，是在定时器中修改的，也可读取
		2. [[值]]：当前读取到的（可能是旧值）
...
1. 会**捕获**定义它的那次渲染中的props与state
	...
	3. 因为监听值没有改变，所以该次更新跳过了该effect，没有重新进行捕获。
形式：useEffect(() => {}, [])
