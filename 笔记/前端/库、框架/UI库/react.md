...
1. 虽然[[函数式编程]]很大程度依赖于*函数的纯粹性*：[[纯函数]]，但有时也必须改变一些事物，这些更改称为[[副作用]]。
	1. react 中副作用通常属于事件处理程序[^6]。
2. react 是围绕[[纯函数]]这个概念设计的。它假设你创建的每个组件都是纯函数，即 react 组件在相同输入时必须返回相同的 [[JSX]]。
3. [[副作用]]
	1. 将外部变量通过 Props 传递【相同输入】，而不是去创造副作用，产生**突变**【组件在渲染时修改了*预先创建*的变量】。[[Pasted image 20230811151830.png]] 
	2. 组件在渲染时修改*渲染时创建*的变量是完全可以的。
4. 尽可能只通过渲染来表达页面，这能让项目走得很远
# 策略
不可变值
组合而非继承
状态提升
