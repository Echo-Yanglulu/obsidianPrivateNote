...
4. 分支：一系列的提交，通常指向最新提交
	...
	2. 修改分支指向
		...
		2. git reset HEAD^ 分支指向上一个提交
			1. 后续变更还在，但不在暂存区
5. HEAD：通常指向分支上最新提交，偶尔直接指向提交。默认指向分支上最新的提交记录
	1. 默认。指向分支【指向提交树上的最新点】
	2. 分离HEAD：指向提交【指向提交树的任意一点】
6. 查找提交
...
9. 合并
	1. merge
	2. rebase
		1. 部分提交：-i  + [[hash值]] ，可**合并**该hash之后（不含）的提交。
		2. 整体提交：分支名
			1. 将当前分支与目标分支对比，将当前分支上从公共commit起之后的**commit移动**到目标分支
