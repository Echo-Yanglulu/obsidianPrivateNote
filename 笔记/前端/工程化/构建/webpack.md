# 概述
定义
	1. 是一个*现代JS应用*的*静态模块*打包工具。以一个或多个入口模块为起点，创建一个依赖关系图。将项目所需的模块组合产出为一个或多个bundle。
	2. 
功能
...
提供了一个hook体系，负责管理**插件的注册与调用** 
![[webpack.svg]] 
## 工作流程
### 原理层面
1. 初始化。主要是**实例化Compiler对象**（以及实例化**多个tapable hook**）
...
### 内容层面
1. 解析入口
	1. 遇到一个 entry 时，Webpack 会尝试使用 entry 的 resolve 配置将 entry 与文件系统中的文件匹配
		1. 如果解析通过失败，Webpack 会引发运行时错误
		2. 正确解析文件，Webpack 将根据 loader 的定义对匹配的文件执行处理
2. 处理各种文件类型
...
2. 功能：转换模块的源码。
	...
	3. 在 JavaScript 模块中 import CSS 文件
3. 特性
	1. 可通过options对象*配置* 
	2. *运行*在 [[node]] 中，可执行任何操作
	3. 支持链式*调用* 
	4. 对一种模块使用的多个loader默认是*倒序执行*的。也可以通过手动定义顺序
	5. 可同步或*异步* 
		1. 同步loader。`module.exports = input => input + input;` 
		2. 异步loader
			1. 与同步的区别：返回值用callback传递出去。
			2. `module.exports = function (input) { const callback = this.async(); callback(null, input + input); }` 
	6. [[babel#plugin（最基础的工具）|插件]]能为loader带来更多特性【插件可对loader进行扩展】
	7. 会产生额外的意外文件
###  分类
1. 文件预处理
...
8. url-loader: 和 file-loader 类似，但是当文件小于设定的 limit 时可以返回一个 Data Url，wepakck5 asset/inline 内置支持。**将文件转为一个Data Uri直接引入页面**。
...
loader无法解决的其他事【太笼统了！】
1. 功能：用户直接介入编译流程
	1. 通过调用插件，**在不同的生命周期hook上注册处理程序。可以在编译过程中的任一时间点执行**
