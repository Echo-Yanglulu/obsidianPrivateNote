# 概述
将ES6代码转换为低版本代码以兼容低版本浏览器。插件化[^18]的语法转义工具，是一个monorepo ，包含了很多子项目。其中有些工具在开发plugin时可用。
![[Pasted image 20220730172031.png]]
# 功能
一个工具链，主要用于语法转换，一种特殊的[[编译器]]，把*高版本*的JS转换为*低版本*的JS代码[^1] ，在低版本的浏览器或其他环境中使用。
	1. 语法转换
	2. polyfill
	3. 修改源码
...
3. 通过**preset-env**的useBuiltlns 与targets智能配置。（一般是core-js 与useBuiltlns 结合使用）
...
大多情况只用这个preset即可，主要是两个配置（引入polyfill 代码）
	1. useBuiltlns：**polyfill代码注入方式** [^9] 
		1. false：不自动注入[^13] 
		2. usage：根据*实际使用*自动注入[^14]![[Pasted image 20220728232655.png]]
		3. entry：根据*环境配置*自动注入![[Pasted image 20220728232437.png]]
...
本质是一组NPM包（使用方式）
	...
	3. 结合[[笔记/前端/工程化/构建/构建#打包工具 工具|打包工具]]使用
