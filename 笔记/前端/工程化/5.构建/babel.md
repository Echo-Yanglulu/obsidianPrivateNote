插件化的语法转义工具，是一个monorepo ，包含了很多子项目。其中有些工具在开发p
# 原理
...
### 遍历
AST的遍历都是深度优先。因为是深度优先，所以走到尽头后会向上返回到一个公共节点[^17]，再向下遍历另一个分支[^16]。
对于一个访问者，babel有两次机会来访问同一个节点：进入与退出。
开发babel插件一般会使用访问者模式这种设计模式，因为遍历AST的过程就是不断访问各个节点的过程，于是babel插件就使用这个模式。![[Pasted image 20220730170953.png]]
访问者模式：每个方法都接收两个参数：
	1. path
