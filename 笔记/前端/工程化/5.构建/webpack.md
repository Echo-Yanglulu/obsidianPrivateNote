...
## loader
定义
	1.  用于转换[^2]==JS/JSON之外==的其他**文件**，得到==JS模块==。是一个接受输入，返回输入的**函数**。因为webpack是基于node.js开发的，node天然支持js，如果有高级语法则需要使用babel-loader。
...
[^1]: 几乎所有功能都由插件提供
[^2]: 将所有格式的文件转换成JS模块：以便webpack==分析依赖关系==，并在浏览器中==加载==。
