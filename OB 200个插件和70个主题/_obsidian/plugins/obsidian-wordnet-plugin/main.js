...
var DictionarySuggester = class extends import_obsidian.FuzzySuggestModal {
  constructor(plugin) {
    super(plugin.app);
    this.plugin = plugin;
    ...
    setTimeout(() => __async(this, null, function* () {
      ...
      if (yield adapter.exists(pathWordNetJson)) {
        ...
        if (navigator.onLine === false) {
          new import_obsidian.Notice("You do not have an internet connection, and the WordNet dictionary cannot be downloaded. Please restore your interent connection and resteart Obsidian", 3e4);
          this.plugin.unload();
        } else {
          const downloadMessage = new import_obsidian.Notice("WordNet dictionary is being downloaded, this may take a few minutes. This message will disappear when the process is complete.", 0);
          try {
            const response = yield fetch("https://wordnet.glitch.me/dict-WordNet.json");
            downloadMessage.hide();
            if (!response.ok) {
              if (response.status === 404) {
                new import_obsidian.Notice(`The WordNet dictionary file is not currently available for download. Please try again later or contact the developer on Twitter: @TfThacker for support.`, 3e4);
              } else {
                console.log(`Error in WordNet dictinary: ${response.status}`);
                new import_obsidian.Notice(`An error has occured with the download, please try again later: ${response.status}`);
              }
              this.plugin.unload();
            } else {
              this.wordNet = yield response.json();
              yield adapter.write(pathWordNetJson, JSON.stringify(this.wordNet));
            }
          } catch (e) {
            console.log(`Error in WordNet dictinary: ${e}`);
            new import_obsidian.Notice(`An error has occured with the download, please try again later: ${e}`);
            this.plugin.unload();
          }
        }
      }
  ...
  onChooseSuggestion(item, evt) {
    ...
    if (currentView != null && currentView.getMode() === "source")
      ...
      new import_obsidian.Notice(item.item.Term + " \n" + item.item.Definition, 1e4);
  }
};
...
var WordNetPlugin = class extends import_obsidian2.Plugin {
...
var WordNetSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setName("Enable Ribbon Support").setDesc("Toggle on and off the WordNet dictionary button in the ribbon.").addToggle((cb) => {
